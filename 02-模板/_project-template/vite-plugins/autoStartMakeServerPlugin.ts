...
export async function registerOfficialProject(projectRoot: string, adminOrigin: string): Promise<RegisteredProject> {
  ...
  // 注册成功后立即把本项目设为 Make 的 active project，避免多项目共享
  // 53817 单例时，新启动的项目只注册未激活，导致首开空白、需手动"切换项目"
  // 才能打开。与工作台 PUT /api/projects/active 逻辑一致且幂等。
  await activateProject(adminOrigin, existing?.id || identity.id).catch(() => {});
...
async function activateProject(adminOrigin: string, projectId: string): Promise<void> {
  const response = await fetch(new URL('/api/projects/active', adminOrigin), {
    method: 'PUT',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ projectId }),
  });
  if (!response.ok) {
    throw new Error(`PUT /api/projects/active failed with ${response.status}`);
  }
}
